...
public class TcpPlugin implements PluginCloseListener, IPlugin {
    ...
    @Autowired
    private YkcTcpConfig tcpConfig;
    ...
    public void init() {
        ...
        try {
            ...
            log.info("插件信息: {} {}", config, tcpConfig);

            ykcTcpVerticle.setConfig(tcpConfig);

            Future<String> future = vertx.deployVerticle(ykcTcpVerticle);
            future.onSuccess((s -> {
                deployedId = s;
